...
!!! success "🧮 実務で使う：[決定計算機](calculator.html){target=_blank}"
    **自分の需要データを貼る（or 平均・SD）＋自分のコスト → 6形式の推奨確保量・供給不足確率・期待コストを表で出し、CSV/印刷で持ち出せます。** 学習デモでなく、実際の「どれだけ確保するか」に使える計算機（計算は手元のブラウザ内）。
