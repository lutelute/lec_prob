---
title: "第3章 — 期待値・分散・共分散・相関・条件付き期待値"
module: 3
type: note
learning_objectives:
...
> この章は **③「何を良しとするか」の数値化**——平均・ばらつき・尾部リスク——を扱います。

第2章で分布（地図全体）を手にしました。ここでは地図を**1つ／少数の数に要約**します。ただし「どの数で要約するか」で、見えるリスクが変わります。本章の背骨は——

> **平均が同じでも、リスクは違う。** だから平均だけでは意思決定できない。

これが 第6章の「期待値最小化 vs CVaR最小化 vs ロバスト」の分岐点になります。
...
指示関数との橋（第1章）：$E[\mathbb{1}_A] = P(A)$。確率は期待値の特別な場合。

### 2.3 イェンゼンの不等式（第0章の罠の正体）
$g$ が凸関数なら
$$
E[g(X)] \ge g(E[X]).
$$
> 第0章の「平均で評価する罠」$E[\text{Cost}(q,D)] \ge \text{Cost}(q,E[D])$ はこれ。
...
> 第6章で「何を最小化するか」を選ぶとは、**この要約量を選ぶこと**にほかなりません。
...
$$
> シナリオ別の期待値を確率で束ねる——これは 第5章のシナリオ法・二段階確率計画の計算そのものです。

---

## 8. VaR と CVaR：尾部リスクの言葉（第6章への橋）
...
> **なぜ CVaR が主役になるか**：① VaR が無視する「境界の先の大損」を平均として捉え、② 数学的に**凸**なので最適化に組み込める（第6章）。
...
| 概念 | 電力での意味 | 後段 |
|---|---|---|
| 期待値 $E[\text{コスト}]$ | 平均運用費 | 期待値最小化（第6章） |
| 分散・$\sigma$ | 運用費・出力の変動 | 予備力サイジング |
| 共分散・相関 | 需要×PV、地点間PVの連動 | 正味負荷リスク、ポートフォリオ |
| 条件付き期待値 | シナリオ別の期待費用 | 二段階確率計画（第5章, 第6章） |
| VaR | 95%ile の需要・損失 | 設計値・チャンス制約 |
| CVaR | 高価格・逼迫時の平均損失 | CVaR最小化（第6章） |

> **設計の勘所**：「平均運用費が同じ2案」を比べるとき、**CVaR で順位が逆転**することがある。
> 何を最小化するか（平均か尾部か）は技術問題ではなく**価値判断**であり、それを数式（目的関数）に落とすのが 第6章です。
...
### 発展
7. VaR と CVaR の違いを、「境界 vs 平均」「凸性」「最適化のしやすさ」の3点で説明し、なぜ 第6章で CVaR が好まれるか論ぜよ。
...
??? question "Q1. 平均が同じ2つの案は「同じ」と言える？"
    言えない。**分散（ばらつき＝リスク）**が違えば、意思決定は変わる。平均だけでは決められない。

??? question "Q2. 2つの不確実量を足すと、分散はどうなる？"
    $\mathrm{Var}(X{+}Y)=\mathrm{Var}X+\mathrm{Var}Y+2\,\mathrm{Cov}(X,Y)$。**正相関で増え、負相関で減る**（相殺）。

## 13. まとめと次の一手

- 期待値は重心（線形性が強力、相関に強い）。イェンゼンが 第0章の罠の正体。
- 分散はばらつきの大きさ（対称、尾部に鈍感）。**平均が同じでもリスクは違う**ので、VaR・CVaR で尾部を測る。
- 共分散・相関は**合成リスク**を左右する（正味負荷）。独立と無相関は別物。
- 条件付き期待値・全期待値の法則は、シナリオ計算（第5章, 第6章）の土台。
