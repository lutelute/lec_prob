...
key_assumptions:
  - この段階では分布は「与えられたもの」として扱う（推定は Module 4）
  - コストは数値化できると仮定する
references:
...
Module 0 のゴールは公式を覚えることではありません。**「なぜ、わざわざ確率という面倒な言語を導入するのか」**に、自分の言葉で答えられるようになることです。
...
> $$ \text{Cost}(q,\, E[D]) \;\neq\; E[\text{Cost}(q, D)]. $$
> これを **平均の落とし穴（flaw of averages）** と呼びます。数学的には**イェンゼンの不等式**（Module 3 で再訪）の現れです。
...
という分位点（需要分布の上位91%点）で決まることを Module 6 で学びます。いまは結論だけ：
...
> - ④は③を**捨てている**のではなく、**離散的に近似**している（Module 5）。
>
...
- 適切な表現：**分布のパラメータの不確かさ**、または**分布の集合（曖昧性集合）** → 分布ロバスト最適化（Module 6）。
...
> これは Module 6 で「どの最適化形式を選ぶか」に直結します。
...
|---|---|---|---|---|
| ① 決定論 | 代表値1つに固定 | 単純・高速 | ばらつき・非対称に脆い | M0, M6 比較対象 |
| ② シナリオ | 有限個の場合を列挙 | 直感的・計算可能 | シナリオ選びが命 | M5 |
| ③ 確率分布 | 分布で期待値・確率を計算 | 平均・リスクを定量化 | 分布を知る必要 | M2–M4, M6 |
| ④ ロバスト | 最悪ケースに備える | 分布不要・安全 | 過度に保守的になりがち | M6 |

> ①→④ に進むほど「安全だが保守的・高コスト」に寄ります。**どこで止めるか**が意思決定者の判断であり、その判断を支えるのが Module 1–6 です。
...
| 発電機・設備故障 | 偶然的（希少事象） | 事象＋確率（M1） | 1件で大規模波及 |
...
> だから「点予測で運用」は、平均の落とし穴に正面からはまります。これが Module 6 までの全体動機です。
...
| 「シナリオを使うのは分布を諦めたから」 | シナリオは分布の**離散近似**。捨てたのではなく、計算可能にしている（M5）。 |
| 「ロバスト（最悪対応）が常に安全で良い」 | 最悪に備えると過剰投資になりうる。安全と費用のトレードオフ（M6）。 |
...
> 「事象」と「確率変数」は何が違うのか——ここを曖昧にしたまま進むと Module 2 以降で必ずつまずきます。
> → [01_events_and_probability](01_events_and_probability.md)
