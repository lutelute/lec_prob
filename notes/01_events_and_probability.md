...
> この章は **②の最初の言語＝「事象」** を厳密にします。

第0章で「確率という言語が要る」と分かりました。その言語の**最小単位**が、本章の主役 **事象（event）** と **確率（probability）** です。ここを曖昧にすると、第2章の確率変数・分布で必ず行き詰まります。

---

## 1. 現象・直感：なぜ「事象」を厳密にするのか

第0章の保護リレー（故障検知）を思い出します。現場で人はこう言います。
...
> **事象は集合である。** これが 第1章で最初に身体化すべき一点です。
...
連続標本空間は 第2章で本格的に扱いますが、「事象＝条件を満たす結果の集合」という捉え方は同じです。
...
$$
は、$A$ が起きたら1・起きなければ0を返す確率変数です。しかも $E[\mathbb{1}_A] = P(A)$ という美しい橋がかかります（期待値は 第3章）。

> **覚え方**：事象は「世界を yes/no で切る集合」、確率変数は「世界に数値を貼る関数」。
> 確率変数を**閾値で切る**と事象になる（$\{X \ge 2\}$）。これが 第2章の区間確率 $P(a \le X \le b)$ の正体です。
...
> この一点が 第2章の核心（密度 ≠ 確率）に直結します。いまは「確率0＝不可能、ではない場面がある」と覚えてください。
...
8. 連続な需要 $D$ について「$P(D=100)=0$ なのに $D$ は必ず何らかの値をとる」を、第0章の意思決定（容量確保）と結びつけて、なぜ点ではなく区間・分布で考えるべきか論ぜよ。
...
自分で答えてから開いてください（[▶ ベイズツール](../interactive/index.md)で確かめる）。

??? question "Q1. 事象と確率変数は何が違う？"
    **事象**は「起こる/起こらない」＝標本空間の部分集合。**確率変数**は各結果に**数を割り当てる関数**。事象は確率変数の値の条件（例 $X>x$）として表せる。

??? question "Q2. 感度99%の検査で陽性。本物と言い切れる？"
    言えない。故障が**稀**なら基準率の誤謬で、事後確率は意外に低い（事前×尤度で決まる）。
...
> 最大の山は「**密度 $f_X(x)$ は確率ではない**」。確率は区間の面積——第1章の「確率0≠不可能」がここで効いてきます。
> → [02_random_variables_and_distributions](02_random_variables_and_distributions.md)
>
