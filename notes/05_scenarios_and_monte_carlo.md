...
key_assumptions:
  - 分布またはサンプラは与えられている（推定は 第4章）
  - SAA は最適化の前段として導入（本格的最適化は 第6章）
references:
...
> この章は **②の4番目の言語＝「有限シナリオ」** と、それを使う計算（モンテカルロ・SAA）を扱います。

分布（第2章–4）は連続で、積分も期待値も**解析的に解けるとは限りません**。そこで分布を**有限個の代表点＋確率**に落とし、和で近似します。本章の背骨は——

> **シナリオは分布を「捨てている」のではない。計算可能にするための「離散近似」である。**

この一点を誤解すると、「シナリオ法＝大雑把な手抜き」と誤り、第6章のシナリオベース確率計画を正しく使えません。

---

## 1. 現象・直感：積分が解けないとき、どうするか

第0章の容量問題を、需要が**連続分布** $D\sim\mathcal{N}(100,15^2)$ の場合で解きたいとします。
期待コスト
$$
...
**2つの近似戦略**：
1. **決定論化**：$D$ を平均100に固定 → 第0章の「平均の罠」に落ちる。✗
...
$$
> これが 第6章で「連続の確率最適化」を「有限個のシナリオ上の最適化（LP/MILP）」に変える仕掛けです。
...
真の最適は臨界比の分位点：$q^\*=F_D^{-1}(10/11)=F_D^{-1}(0.909)=120.03$（**第0章の2点版 $q^\*=120$ の連続版！**）。
...
> 第0章で「平均で最適化すると120ではなく100を選ぶ罠」を見た。SAA は**サンプルを通じて分布の非対称性を正しく拾い**、$q\approx120$ に到達します。
...
> 「平均運用費は推定できたが、稀な大停電確率は実は推定できていない」——これが 第6章のチャンス制約・CVaR をサンプルで解くときの落とし穴。
...
> この「木」が 第6章の**二段階確率計画**そのもの。第1段＝今の決定（蓄電池容量・起動停止）、第2段＝不確実性が判明した後の調整（充放電・出力調整）。
...
> **設計の勘所**：シナリオ生成は **第4章（データ→分布）の質**に依存する。
