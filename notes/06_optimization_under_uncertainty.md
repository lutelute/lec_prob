...
本教材の**到達点**です。第0章–5 で積み上げた確率の言語を使い、**同じ1つの意思決定問題**を6つの形式で定式化し、**なぜこの形式を選ぶのか**を自分の言葉で言えるようにします。
...
- **いつ妥当**：ばらつきが小さい／結果が対称なとき（第0章の「壊れない条件」）。
...
- **意味**：右裾の面積（第2章の超過確率！）を $\varepsilon$ 以下に抑える。**「9割の日は供給支障なし」**を直接保証。
...
- **意味**：チャンス制約が「違反**確率**」を見るのに対し、CVaR は「違反したときの**深さ**まで」平均で見る。**凸**なので最適化に乗る（第3章 §8）。
- **いつ妥当**：**大損（破滅的損失）を避けたい**とき。尾の深さが効く非対称・回復不能なリスク。
- **代償**：分布／多数サンプルが要る。$\alpha$ の選択が価値判断。希少事象の尾は推定が難しい（第5章 §5）。
...
- **意味**：分布の**形までは信じない**（平均・分散などモーメントだけ）。第0章の**認識論的不確実性**への正攻法。
...
- $c_s$ を下げる（非対称を弱める）と全形式が100へ寄る（第0章の罠が消える）。
