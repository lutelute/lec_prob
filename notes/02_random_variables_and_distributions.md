...
!!! abstract "30秒まとめ"
    - **何の話か**：PMF・PDF・CDF。密度（高さ）と確率（面積）の違い。
    - **分かること**：連続では 確率＝区間の面積＝CDFの差。密度は1を超えてよい。
    - **使う場面**：需要・出力など連続量の「ある範囲に入る確率」を出すとき。 → [▶ PDF・CDFツール](../interactive/index.md) で面積を動かす。

> **5つの問い**：①何が不確実か ②どの言語で表すか ③何を良しとするか ④式のどこに出るか ⑤代償は何か。
> この章は **②の中核言語＝「確率変数とその分布」** を厳密にします。

第1章で「事象＝集合」を学びました。ここでは主役を **確率変数** に移し、その振る舞い全体を **分布**（PMF・PDF・CDF）で記述します。最大の山は——

> **密度 $f_X(x)$ は確率ではない。** 確率は区間の**面積**である。

これを身体化できれば、第6章のチャンス制約・CVaR まで一直線です。
...
第0章で「明日の需要は1つの数では足りず、分布が要る」と分かりました。では分布とは何を持っている情報なのか。

- 「需要が **ちょうど** 100 になる確率は？」→ 連続量では、これは **0**（第1章 §8）。
...
$$
> 事象（集合）と違い、確率変数は**数値**を返す。第1章 §4 の通り、確率変数を閾値で切ると事象になる：$\{X \le x\} = \{\omega : X(\omega)\le x\}$。
...
連続変数では $P(X=x)=0$（第1章 §8）。だから「点の確率」では分布を記述できません。代わりに **確率密度関数 (PDF)** を使います。
$$
...
> **裾の重さ**に注意：正規は裾が軽く、価格や需要のスパイクを**過小評価**しがち。右に重い裾には対数正規など（第1ツールで比較可）。第3章・4 で再訪。

---

## 7. ヒストグラムと分布の関係（第4章への橋）

手元の**データ**からヒストグラムを描き、面積が1になるよう正規化すると、PDF の**推定**になります。
- ヒストグラム＝**観測の要約**（有限データ、でこぼこ）。
- PDF＝**母集団の仮定**（理想化、なめらか）。
- 両者は**一致しない**（データはばらつく）。この区別が 第4章の主題「観測データ ≠ 真の分布」。

> いまは「分布は与えられた」として扱いますが、現実には分布は**データから推定**します。そこには別の不確実性（認識論的、第0章）が乗ることを覚えておいてください。
...
|---|---|---|---|
| 需要・PV出力 | 正規／対数正規 | $P(X>$容量$)$＝超過確率 | チャンス制約（第6章） |
| $n$ 台中の故障台数 | 二項 | $P(X\ge k)$ | $N{-}1$ 信頼度 |
| 年間故障件数 | ポアソン | $P(X\ge1)$ | 保全計画 |
| 故障間隔・待ち時間 | 指数 | $P(X>t)$＝信頼度 | 信頼性評価 |
| 価格 | 対数正規（重い裾） | 尾部の確率 | CVaR（第6章） |

> **設計の勘所**：「超過確率 $P(X>c)$」は右裾の面積であり、第6章のチャンス制約 $P(g(x,\xi)\le0)\ge1-\varepsilon$ の中身そのもの。
...
自分で答えてから開いてください（[▶ PDF・CDFツール](../interactive/index.md)で面積を動かす）。

??? question "Q1. 密度 f(x)=2 は「確率が2」という意味？"
    違う。密度は確率ではない。確率は**区間の面積**（$\int_a^b f\,dx$）で、1点の確率は0、密度は1を超えてよい。

??? question "Q2. CDF $F(x)$ は何を表す？区間確率とどうつながる？"
    $F(x)=P(X\le x)$。区間確率は**CDFの差** $P(a\le X\le b)=F(b)-F(a)$。
