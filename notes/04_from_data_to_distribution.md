...
> この章は **⑤の核心＝「分布はどこから来るのか／どこまで信じられるか」** を扱います。

第2章–3 では「分布は与えられた」としてきました。けれど現実には、**真の分布を直接見ることはできません**。見えるのは**有限のデータ**だけ。本章の背骨は——

> **観測データは真の分布ではない。** データは真の分布から漏れ出た**有限の標本**にすぎない。

この区別（第0章の「認識論的不確実性」）が、第6章で「分布を信じる（期待値・CVaR）か、信じない（ロバスト）か」を決めます。
...
> ECDF は真のCDFの**素直な推定**で、第5章の経験分布サンプリングの土台。
...
> これは 第6章の「分布を信じる最適化 vs ロバスト最適化」の判断と**同じ構造**。
...
第2章–3 の多くは **iid（独立同分布）** を暗黙に仮定しています。しかし実データは違う。
...
- この誤差分布が、第5章のシナリオ生成、第6章のチャンス制約・CVaR の**入力**になる。

> 第0章の「予測値（点）→ 確率分布」への格上げが、ここで具体化します。
...
> データから作った分布は**いつの・どの条件の分布か**を常に問う。これが 第6章で「分布を信じるか（期待値）／集合で守るか（ロバスト）」の判断に直結。
...
8. 予測誤差の分布を「リード時間別・季節別」に分けて推定すべき理由を、iid 仮定とチャンス制約（第6章）への影響から説明せよ。
